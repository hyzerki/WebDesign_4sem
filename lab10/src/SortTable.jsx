...
function Catalog(props) {
    ...
    return (
        <div>
            <Search catalog={props.tableData} />
            <div>
                ...
                {props.tableData.map(element => (
                    <div key={JSON.stringify(element)} style={{ display: "flex" }}>
                        ...
                        <div>
                            ...
                            <div>{element.discount !== 0 ? <span style={{ textDecoration: "line-through" }}> {element.price * (1 - (element.discount/100))} </span> : null}  {element.price}</div>
...
function Search(props) {
    ...
    const accurateField = React.useRef(null);

    function find(){
        let search = searchField.current.value;
        let temp;
        if(accurateField.current.isChecked){
            temp = props.catalog.filter(e=> e.name === search);
        }else{
            temp = props.catalog.filter(e=> e.name.includes(search));
        }
        setResult(temp);
        console.log(temp);
    }
    ...
    return (
        <div>
            <input type="text" ref={searchField}/> <input ref={accurateField} type="checkbox" />Точное совпадение
            <input type="button" onClick={find} value="Поиск" />
            <div>
                {result.map(element => (
                    <div key={JSON.stringify(element)} style={{ display: "flex" }}>
                        <div>
